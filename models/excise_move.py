...
class excise_move(models.Model):
    ...
    move_source_address = fields.Many2one('res.partner', string='Sender', compute='_compute_addresses', store=True)
    move_destination_address = fields.Many2one('res.partner', string='Receiver', compute='_compute_addresses', store=True)
    ...
    move_excise_stock_type = fields.Selection([
        ...
        related='stock_move_id.product_id.excise_stock_type', index=True, readonly=True, store=True)
